...
#define foreach_upf_encap_error _ (ENCAPSULATED, "good packets encapsulated")
  ...
  if (is_ip4)
    {
      ...
      clib_memcpy_fast (&tlv_ie->data, &error->addr.ip4,
                        sizeof (ip4_address_t));
    }
  else
    {
      tlv_ie->len = clib_host_to_net_u16 (sizeof (ip6_address_t));
      clib_memcpy_fast (&tlv_ie->data, &error->addr.ip6,
  ...
  if (is_ip4)
    {
      ...
      p0->current_length +=
        sizeof (ip4_header_t) + sizeof (udp_header_t) + sizeof (gtpu_header_t);
      ...
      vlib_buffer_advance (
        b0, -(uword) (sizeof (ip4_header_t) + sizeof (udp_header_t)));
      ...
      gtpu1->length = clib_host_to_net_u16 (len_p1 + (ext_header_len - 1) +
                                            (sizeof (gtpu_header_t) - 8));
      ...
      sum1 =
        ip_csum_update (sum1, ip41->ttl, host_config_ttl, ip4_header_t, ttl);
      ip41->ttl = host_config_ttl;

      new_ip_len1 = clib_host_to_net_u16 (new_len1);
      sum1 =
        ip_csum_update (sum1, ip41->length, new_ip_len1, ip4_header_t, length);
      ...
      udp1->length = clib_host_to_net_u16 (
        sizeof (udp_header_t) + gtp_len_encoded + len_p1 + ext_header_len);
      /* UDP checksum. */
      udp1->checksum = 0;
      ...
      vlib_buffer_advance (p0, -(uword) (sizeof (ip4_header_t) +
                                         sizeof (udp_header_t) +
                                         gtp_len_encoded + ext_header_len));
  ...
  else
    {
      ...
      /* Set buffer borders to include headers for now */
      p0->current_length +=
        sizeof (ip6_header_t) + sizeof (udp_header_t) + sizeof (gtpu_header_t);
      ...
      vlib_buffer_advance (
        b0, -(uword) (sizeof (ip6_header_t) + sizeof (udp_header_t)));
      ...
      udp1->checksum = 0;
      udp1->checksum =
        ip6_tcp_udp_icmp_compute_checksum (vm, p0, ip61, &bogus);
      if (udp1->checksum == 0)
        udp1->checksum = 0xffff;
      ...
      vlib_buffer_advance (p0, -(uword) (sizeof (ip6_header_t) +
                                         sizeof (udp_header_t) +
                                         gtp_len_encoded + ext_header_len));
  ...
  if (is_ip4)
    {
      ...
      sum0 = ip_csum_update (sum0, 0, new_l0, ip4_header_t,
      ...
      while (n_left_from >= 8 && n_left_to_next >= 4)
        {
          ...
          pdr0 =
            vec_elt_at_index (r0->pdr, upf_buffer_opaque (b0)->gtpu.pdr_idx);
          pdr1 =
            vec_elt_at_index (r1->pdr, upf_buffer_opaque (b1)->gtpu.pdr_idx);
          pdr2 =
            vec_elt_at_index (r2->pdr, upf_buffer_opaque (b2)->gtpu.pdr_idx);
          pdr3 =
            vec_elt_at_index (r3->pdr, upf_buffer_opaque (b3)->gtpu.pdr_idx);
          ...
          if (PREDICT_FALSE ((upf_buffer_opaque (b1)->gtpu.hdr_flags &
                              GTPU_E_S_PN_BIT) != 0))
            vlib_buffer_advance (b1,
                                 -upf_buffer_opaque (b1)->gtpu.ext_hdr_len);
          if (PREDICT_FALSE ((upf_buffer_opaque (b2)->gtpu.hdr_flags &
                              GTPU_E_S_PN_BIT) != 0))
            vlib_buffer_advance (b2,
                                 -upf_buffer_opaque (b2)->gtpu.ext_hdr_len);
          if (PREDICT_FALSE ((upf_buffer_opaque (b3)->gtpu.hdr_flags &
                              GTPU_E_S_PN_BIT) != 0))
...
#undef _
              ...
              if (udp1->checksum == 0)
                udp1->checksum = 0xffff;
              udp2->checksum =
                ip6_tcp_udp_icmp_compute_checksum (vm, b2, ip6_2, &bogus);
              if (udp2->checksum == 0)
                udp2->checksum = 0xffff;
              udp3->checksum =
                ip6_tcp_udp_icmp_compute_checksum (vm, b3, ip6_3, &bogus);
              if (udp3->checksum == 0)
